---
geometry: "left=1.3cm,right=2cm,top=0.8cm,bottom=2cm"
output: pdf_document
---

\underline{NOTA}: Per tutti i codici SQL abbiamo usato Postgre versione 14.5, in quanto la 9.6 specificata a inizio corso non accettava la sintassi dei trigger per come l'abbiamo studiata.
...
![Diagramma ER](../parte_1/deliverable/media/social_market_v2.drawio.png)
...
- `Clienti`
  ...
  - il saldo punti deve essere compreso tra 0 e i punti mensili (inclusi gli estremi)
...
- `Appuntamenti`
  ...
  - 'saldo_iniziale' deve essere maggiore o uguale di 0
...
- `Fasce orarie`
  - 'ID' e' univoco
  - 'giorno', 'ora inizio' e 'ora fine' sono univoci insieme
  - 'giorno' deve ovviamente essere uno tra i giorni della settimana 

- `Turni`
  - 'ID' e' univoco
  - 'data', 'ora_inizio' e 'ora_fine' sono univoci insieme
...
- `Acquisto`
  - 'importo speso' deve essere maggiore di 0
...
![Diagramma ER ristrutturato](../parte_1/deliverable/media/social_market_v2_ristrutturato.drawio.png){width=98%}
...
#### `modifiche effettuate nella ristrutturazione`
...
- Volontari
  ...
  - $email \to ID$
- Fasce orarie
  - $ID \to giorno, ora\_inizio, ora\_fine$
- Turni
...
\underline{NOTA}: "$ID \to ...$" indica che l'ID implica tutti gli altri attributi della relazione (chiave)